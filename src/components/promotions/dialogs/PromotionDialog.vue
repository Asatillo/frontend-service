                ...
                <v-container class="pa-0">
                    <v-row>
                        ...
                        <v-col class="py-0" cols="4">
                            <v-text-field v-model="editedItem.amount" label="Amount" required
                        ...
                        <v-col class="py-0" cols="6">
                            <v-text-field v-model="editedItem.endDate" label="End Date" required
                                type="date"></v-text-field>
                        </v-col>
...
defineExpose({ openNewPromotionDialog, openChangePromotionDialog });
const emit = defineEmits(['update-table-after-dialog']);
...
function save() {
    if (editedItem.value.id) {
        editPromotion(editedItem.value).then(() => {
            emit('update-table-after-dialog');
            close();
        });
    } else {
        addPromotion(editedItem.value).then(() => {
            emit('update-table-after-dialog');
            close();
        });
    }
